// Optimized craftax kernels.
// Bitwise-identical obs/rewards/dones vs Oracle. Optimizations:
//   1. Skip initial map clear in generate_world (every cell written by gen loop).
//   2. Track has_diamond inline during worldgen (remove 4096-cell post-scan).
//   3. Early-exit do_crafting for non-craft actions.
//   4. build_observation: zero slab + single indexed write for block one-hot.
//   5. build_observation: invert mob scan (loop mobs once, stamp cells).
//   6. Achievements as uint32_t bitmask; reward diff via __popc.
//   7. Drop unused combined step_kernel.
#include "craftax_opt3.cuh"
#include <cmath>
...
__device__ void generate_world(EnvState& s, uint64_t seed, int env_id) {
    ...
    // OPT 1: skip the redundant GRASS clear — every cell is written by the loop below.
    ...
    for (int r = 0; r < MAP_SIZE; r++) {
        for (int c = 0; c < MAP_SIZE; c++) {
            ...
            if (blk == BLK_DIAMOND) has_diamond = true;  // OPT 2
...
__device__ void do_crafting(EnvState& s, int action) {
    // OPT 3: skip the two is_near_block scans when action can't be a craft action.
...
// OPT 4+5: zero block/mob slab once, single indexed writes, stamp mobs in one pass.
__device__ void build_observation(const EnvState& s, float* obs) {
    int pr = s.player_r, pc = s.player_c;

    const int N_CELLS = OBS_MAP_ROWS * OBS_MAP_COLS;   // 63
    const int CELL_STRIDE = OBS_MAP_CHANNELS;          // 21
    const int N_MAP = N_CELLS * CELL_STRIDE;           // 1323
    ...
    auto stamp = [&](int r, int c, int chan) {
        int dr = r - pr, dc = c - pc;
...
extern "C" __global__ void step_only_kernel(
    ...
    uint32_t old_ach = s.achievements;  // OPT 6: one u32 save
...
extern "C" __global__ void autoreset_obs_kernel(
    ...
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= num_envs) return;
    if (dones[idx]) generate_world(states[idx], reset_seed, idx + num_envs);
    build_observation(states[idx], obs + idx * OBS_DIM);
}
