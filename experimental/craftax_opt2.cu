// opt2 game logic: takes Ctx (SoA map view). Bitwise-identical outputs to Oracle.
...
__device__ void generate_world(const Ctx& c, uint64_t seed, int rng_seq) {
    EnvState& s = *c.s;
    curand_init(seed, rng_seq, 0, &s.rng);
    ...
    for (int r = 0; r < MAP_SIZE; r++) {
        for (int col = 0; col < MAP_SIZE; col++) {
            ...
            float dist = sqrtf((float)((r-center)*(r-center) + (col-center)*(col-center)));
            ...
            if (blk == BLK_STONE) {
                ...
                if (ore_roll < 0.005f && mountain_val > 0.8f) blk = BLK_DIAMOND;
                else if (ore_roll < 0.035f) blk = BLK_IRON;
                else if (ore_roll < 0.075f) blk = BLK_COAL;
            }
    ...
    map_set(c, center, center, BLK_GRASS);

    if (!has_diamond) {
        for (int attempts = 0; attempts < 1000; attempts++) {
            int r = rand_int(&s.rng, MAP_SIZE);
            int col = rand_int(&s.rng, MAP_SIZE);
            if (map_get(c, r, col) == BLK_STONE) {
                map_set(c, r, col, BLK_DIAMOND);
                break;
            }
        }
    }
...
__device__ void do_crafting(const Ctx& c, int action) {
    if (action < ACT_MAKE_WOOD_PICK || action > ACT_MAKE_IRON_SWORD) return;
    EnvState& s = *c.s;
    bool near_table = is_near_block(c, BLK_TABLE);
    bool near_furnace = is_near_block(c, BLK_FURNACE);

    if (action == ACT_MAKE_WOOD_PICK && near_table && s.inv[0] >= 1) {
        s.inv[0]--; s.inv[6]++; ach_set(s, ACH_MAKE_WOOD_PICK);
    }
    if (action == ACT_MAKE_STONE_PICK && near_table && s.inv[0] >= 1 && s.inv[1] >= 1) {
        s.inv[0]--; s.inv[1]--; s.inv[7]++; ach_set(s, ACH_MAKE_STONE_PICK);
    }
    if (action == ACT_MAKE_IRON_PICK && near_table && near_furnace &&
        ...
        s.inv[8]++; ach_set(s, ACH_MAKE_IRON_PICK);
    }
    if (action == ACT_MAKE_WOOD_SWORD && near_table && s.inv[0] >= 1) {
        s.inv[0]--; s.inv[9]++; ach_set(s, ACH_MAKE_WOOD_SWORD);
    }
    if (action == ACT_MAKE_STONE_SWORD && near_table && s.inv[0] >= 1 && s.inv[1] >= 1) {
        s.inv[0]--; s.inv[1]--; s.inv[10]++; ach_set(s, ACH_MAKE_STONE_SWORD);
    }
    if (action == ACT_MAKE_IRON_SWORD && near_table && near_furnace &&
        s.inv[0] >= 1 && s.inv[1] >= 1 && s.inv[3] >= 1 && s.inv[2] >= 1) {
        s.inv[0]--; s.inv[1]--; s.inv[3]--; s.inv[2]--;
        s.inv[11]++; ach_set(s, ACH_MAKE_IRON_SWORD);
    }
}

__device__ void do_action(const Ctx& c) {
    EnvState& s = *c.s;
    ...
    int8_t blk = map_get(c, tr, tc);
    switch (blk) {
        case BLK_TREE:
            map_set(c, tr, tc, BLK_GRASS);
            s.inv[0] = min_i(9, s.inv[0] + 1);
            ach_set(s, ACH_COLLECT_WOOD);
            break;
        case BLK_STONE:
            if (s.inv[6] > 0 || s.inv[7] > 0 || s.inv[8] > 0) {
                map_set(c, tr, tc, BLK_PATH);
                s.inv[1] = min_i(9, s.inv[1] + 1);
                ach_set(s, ACH_COLLECT_STONE);
            }
            break;
        case BLK_COAL:
            if (s.inv[6] > 0 || s.inv[7] > 0 || s.inv[8] > 0) {
        ...
        case BLK_DIAMOND:
            if (s.inv[8] > 0) {
                ...
                ach_set(s, ACH_COLLECT_DIAMOND);
            }
            break;
        case BLK_GRASS:
            if (rand_f(&s.rng) < 0.1f) {
                s.inv[5] = min_i(9, s.inv[5] + 1);
                ach_set(s, ACH_COLLECT_SAPLING);
            }
            break;
        case BLK_WATER:
            s.drink = min_i(9, s.drink + 1);
            s.thirst = 0;
            ach_set(s, ACH_COLLECT_DRINK);
            break;
        case BLK_RIPE_PLANT:
            map_set(c, tr, tc, BLK_PLANT);
            s.food = min_i(9, s.food + 4);
            s.hunger = 0;
            ach_set(s, ACH_EAT_PLANT);
...
__device__ void place_block(const Ctx& c, int action) {
    EnvState& s = *c.s;
    int tr = s.player_r + DIR_DR[s.player_dir];
    int tc = s.player_c + DIR_DC[s.player_dir];
    if (!in_bounds(tr, tc)) return;
    ...
    if (action == ACT_PLACE_TABLE && s.inv[0] >= 2 && !is_solid(blk)) {
        map_set(c, tr, tc, BLK_TABLE); s.inv[0] -= 2;
        ach_set(s, ACH_PLACE_TABLE);
    }
    else if (action == ACT_PLACE_FURNACE && s.inv[1] >= 1 && !is_solid(blk)) {
        map_set(c, tr, tc, BLK_FURNACE); s.inv[1] -= 1;
        ach_set(s, ACH_PLACE_FURNACE);
    }
    else if (action == ACT_PLACE_STONE && s.inv[1] >= 1 && (!is_solid(blk) || blk == BLK_WATER)) {
        map_set(c, tr, tc, BLK_STONE); s.inv[1] -= 1;
        ach_set(s, ACH_PLACE_STONE);
    }
    else if (action == ACT_PLACE_PLANT && s.inv[5] >= 1 && blk == BLK_GRASS) {
        map_set(c, tr, tc, BLK_PLANT); s.inv[5] -= 1;
        ach_set(s, ACH_PLACE_PLANT);
        for (int i = 0; i < MAX_PLANTS; i++) {
            if (!s.plant_mask[i]) {
...
__device__ void move_player(const Ctx& c, int action) {
    EnvState& s = *c.s;
    ...
    if (is_solid(map_get(c, nr, nc))) return;
    if (has_mob_at(s, nr, nc)) return;
    s.player_r = nr; s.player_c = nc;
}

__device__ bool can_move_mob(const Ctx& c, int r, int col) {
    const EnvState& s = *c.s;
    if (!in_bounds(r, col)) return false;
    int8_t blk = map_get(c, r, col);
    if (is_solid(blk)) return false;
    if (blk == BLK_LAVA) return false;
    if (has_mob_at(s, r, col)) return false;
    if (r == s.player_r && col == s.player_c) return false;
    return true;
}

__device__ void update_mobs(const Ctx& c) {
    EnvState& s = *c.s;
    ...
    for (int i = 0; i < MAX_ZOMBIES; i++) {
        ...
        if (can_move_mob(c, nr, nc)) { s.zombie_r[i] = nr; s.zombie_c[i] = nc; }
    }
    ...
    for (int i = 0; i < MAX_COWS; i++) {
        ...
        if (d < 4) {
            ...
            if (can_move_mob(c, nr, nc)) { s.cow_r[i] = nr; s.cow_c[i] = nc; }
        }
    }
    ...
    for (int i = 0; i < MAX_SKELETONS; i++) {
        ...
        if (can_move_mob(c, nr, nc)) { s.skel_r[i] = nr; s.skel_c[i] = nc; }
    }
    ...
    for (int i = 0; i < MAX_ARROWS; i++) {
        ...
        int8_t blk = map_get(c, nr, nc);
        if (is_solid(blk) && blk != BLK_WATER) {
            if (blk == BLK_FURNACE || blk == BLK_TABLE) map_set(c, nr, nc, BLK_PATH);
...
__device__ void spawn_mobs(const Ctx& c) {
    EnvState& s = *c.s;
    int pr = s.player_r, pc = s.player_c;
    ...
    auto try_spawn = [&](int min_dist, int max_dist, bool need_grass, bool need_path, int* out_r, int* out_c) -> bool {
        for (int attempts = 0; attempts < 20; attempts++) {
            int r = rand_int(&s.rng, MAP_SIZE);
            int col = rand_int(&s.rng, MAP_SIZE);
            int dist = l1_dist(r, col, pr, pc);
            if (dist < min_dist || dist >= max_dist) continue;
            if (has_mob_at(s, r, col)) continue;
            if (r == pr && col == pc) continue;
            int8_t blk = map_get(c, r, col);
    ...
    if (n_cows < MAX_COWS && rand_f(&s.rng) < 0.1f) {
        int r, col;
        if (try_spawn(3, MOB_DESPAWN_DIST, true, false, &r, &col)) {
            for (int i = 0; i < MAX_COWS; i++) {
                if (!s.cow_mask[i]) {
                    s.cow_mask[i] = true; s.cow_r[i] = r; s.cow_c[i] = col; s.cow_hp[i] = 3;
                    break;
                }
            }
        }
    ...
    if (n_zombies < MAX_ZOMBIES && rand_f(&s.rng) < zombie_chance) {
        int r, col;
        if (try_spawn(9, MOB_DESPAWN_DIST, false, false, &r, &col)) {
            for (int i = 0; i < MAX_ZOMBIES; i++) {
                if (!s.zombie_mask[i]) {
                    s.zombie_mask[i] = true; s.zombie_r[i] = r; s.zombie_c[i] = col;
    ...
    if (n_skels < MAX_SKELETONS && rand_f(&s.rng) < 0.05f) {
        int r, col;
        if (try_spawn(9, MOB_DESPAWN_DIST, false, true, &r, &col)) {
            for (int i = 0; i < MAX_SKELETONS; i++) {
                if (!s.skel_mask[i]) {
                    s.skel_mask[i] = true; s.skel_r[i] = r; s.skel_c[i] = col;
...
__device__ void update_plants(const Ctx& c) {
    EnvState& s = *c.s;
    for (int i = 0; i < MAX_PLANTS; i++) {
        if (!s.plant_mask[i]) continue;
        s.plant_age[i]++;
        if (s.plant_age[i] >= 600) {
            int r = s.plant_r[i], col = s.plant_c[i];
            if (in_bounds(r, col) && map_get(c, r, col) == BLK_PLANT) {
                map_set(c, r, col, BLK_RIPE_PLANT);
            }
        }
    }
}
...
__device__ void build_observation(const Ctx& c, float* obs) {
    const EnvState& s = *c.s;
    int pr = s.player_r, pc = s.player_c;

    const int N_CELLS = OBS_MAP_ROWS * OBS_MAP_COLS;
    const int CELL_STRIDE = OBS_MAP_CHANNELS;
    const int N_MAP = N_CELLS * CELL_STRIDE;

    #pragma unroll 4
    for (int i = 0; i < N_MAP; i++) obs[i] = 0.0f;

    for (int dr = -3; dr <= 3; dr++) {
        for (int dc = -4; dc <= 4; dc++) {
            int r = pr + dr, col = pc + dc;
            int8_t blk = (in_bounds(r, col)) ? map_get(c, r, col) : BLK_OUT_OF_BOUNDS;
            int cell = (dr + 3) * OBS_MAP_COLS + (dc + 4);
            obs[cell * CELL_STRIDE + blk] = 1.0f;
        }
    }

    auto stamp = [&](int r, int col, int chan) {
        int dr = r - pr, dc = col - pc;
        if (dr < -3 || dr > 3 || dc < -4 || dc > 4) return;
        if (!in_bounds(r, col)) return;
        int cell = (dr + 3) * OBS_MAP_COLS + (dc + 4);
        obs[cell * CELL_STRIDE + NUM_BLOCK_TYPES + chan] = 1.0f;
    };
    for (int i = 0; i < MAX_ZOMBIES; i++)
        if (s.zombie_mask[i]) stamp(s.zombie_r[i], s.zombie_c[i], 0);
    for (int i = 0; i < MAX_COWS; i++)
        if (s.cow_mask[i]) stamp(s.cow_r[i], s.cow_c[i], 1);
    for (int i = 0; i < MAX_SKELETONS; i++)
        if (s.skel_mask[i]) stamp(s.skel_r[i], s.skel_c[i], 2);
    for (int i = 0; i < MAX_ARROWS; i++)
        if (s.arrow_mask[i]) stamp(s.arrow_r[i], s.arrow_c[i], 3);
...
// ========================================================================
// OPT4: __vminu4 clamp. Inventory is 12 bytes = 3 uint32s. Treat as packed
// u8x4 and clamp to 9 with one SIMD op each. Equivalent to min_i(inv[i], 9)
// for inv[i] in [0,255]; since inv values are always non-negative bytes,
// result is bitwise-identical to the previous per-byte clamp.
// ========================================================================
__device__ __forceinline__ void clamp_inventory(EnvState& s) {
...
extern "C" __global__ void reset_kernel(
    EnvState* states, uint8_t* maps, float* obs, int num_envs, uint64_t seed
) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= num_envs) return;
    Ctx c{&states[idx], maps, num_envs, idx};
    generate_world(c, seed, idx);
    build_observation(c, obs + idx * OBS_DIM);
}

extern "C" __global__ void step_only_kernel(
    EnvState* states, uint8_t* maps, const int32_t* actions,
    float* rewards, int8_t* dones, int num_envs
) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= num_envs) return;
    Ctx c{&states[idx], maps, num_envs, idx};
    ...
    do_crafting(c, action);
    if (action == ACT_DO) do_action(c);
    if (action >= ACT_PLACE_STONE && action <= ACT_PLACE_PLANT) place_block(c, action);
    move_player(c, action);
    update_mobs(c);
    spawn_mobs(c);
    update_plants(c);
    update_intrinsics(s, actions[idx]);

    clamp_inventory(s);  // OPT4
    ...
    if (in_bounds(s.player_r, s.player_c) && map_get(c, s.player_r, s.player_c) == BLK_LAVA)
        done = true;
    dones[idx] = done ? 1 : 0;
}

extern "C" __global__ void autoreset_obs_kernel(
    EnvState* states, uint8_t* maps, const int8_t* dones,
    float* obs, int num_envs, uint64_t reset_seed
) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= num_envs) return;
    Ctx c{&states[idx], maps, num_envs, idx};
    if (dones[idx]) generate_world(c, reset_seed, idx + num_envs);
    build_observation(c, obs + idx * OBS_DIM);
}
