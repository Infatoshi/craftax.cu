// opt2: SoA map layout + bitmask is_solid + __vminu4 clamp + block=64 launches.
// Map lives outside EnvState in a separate buffer with layout
//   maps[cell_idx * num_envs + env_idx]
// so adjacent threads (adjacent env_idx) access adjacent bytes -> coalesced.
#pragma once
#include <cstdint>
...
// EnvState WITHOUT the 2048-byte packed map -- map lives in a separate SoA buffer.
...
// Context bundle: state ref + SoA map base + stride + this env's index.
// Passed by value; small enough to stay in registers.
struct Ctx {
    EnvState* s;
    uint8_t* maps;
    int num_envs;
...
__device__ __forceinline__ int8_t map_get(const Ctx& c, int r, int col) {
    int cell = r * MAP_PACKED_ROW + (col >> 1);
    uint8_t byte = c.maps[cell * c.num_envs + c.env_idx];
    return (col & 1) ? (byte >> 4) : (byte & 0x0F);
}

__device__ __forceinline__ void map_set(const Ctx& c, int r, int col, int8_t val) {
    int cell = r * MAP_PACKED_ROW + (col >> 1);
    int off = cell * c.num_envs + c.env_idx;
    uint8_t byte = c.maps[off];
    if (col & 1)
        c.maps[off] = (byte & 0x0F) | ((val & 0x0F) << 4);
    else
        c.maps[off] = (byte & 0xF0) | (val & 0x0F);
}

__device__ __forceinline__ bool ach_get(const EnvState& s, int i) {
    return (s.achievements >> i) & 1u;
}
__device__ __forceinline__ void ach_set(EnvState& s, int i) {
    s.achievements |= (1u << i);
}

__device__ __forceinline__ bool in_bounds(int r, int col) {
    return (unsigned)r < MAP_SIZE && (unsigned)col < MAP_SIZE;
}

// OPT3: bitmask lookup replaces 10-way OR chain.
__device__ __forceinline__ bool is_solid(int8_t blk) {
    constexpr uint32_t SOLID_MASK =
        (1u << BLK_WATER) | (1u << BLK_STONE) | (1u << BLK_TREE) |
        (1u << BLK_COAL)  | (1u << BLK_IRON)  | (1u << BLK_DIAMOND) |
        (1u << BLK_TABLE) | (1u << BLK_FURNACE) |
        (1u << BLK_PLANT) | (1u << BLK_RIPE_PLANT);
    return (SOLID_MASK >> (unsigned)blk) & 1u;
}
...
__device__ __forceinline__ bool has_mob_at(const EnvState& s, int r, int col) {
    for (int i = 0; i < MAX_ZOMBIES; i++)
        if (s.zombie_mask[i] && s.zombie_r[i] == r && s.zombie_c[i] == col) return true;
    for (int i = 0; i < MAX_COWS; i++)
        if (s.cow_mask[i] && s.cow_r[i] == r && s.cow_c[i] == col) return true;
    for (int i = 0; i < MAX_SKELETONS; i++)
        if (s.skel_mask[i] && s.skel_r[i] == r && s.skel_c[i] == col) return true;
    return false;
}

__device__ bool is_near_block(const Ctx& c, int8_t blk_type) {
    int pr = c.s->player_r, pc = c.s->player_c;
    const int dr8[8] = {0, 0, -1, 1, -1, -1, 1, 1};
    const int dc8[8] = {-1, 1, 0, 0, -1, 1, -1, 1};
    for (int i = 0; i < 8; i++) {
        int nr = pr + dr8[i], nc = pc + dc8[i];
        if (in_bounds(nr, nc) && map_get(c, nr, nc) == blk_type) return true;
    }
    return false;
}
