...
// ============================================================
static int g_craftax_wg_force_scalar = 0;

static inline void craftax_smoothworld_classify_scalar(
...
static inline void craftax_smoothworld_ore_scalar(
    const CraftaxSmoothGenConfig* config,
    int ore_index,
...
static inline void craftax_smoothworld_lava_scalar(
    const CraftaxSmoothGenConfig* config,
...
static inline __m512i craftax_wg_mix64_v(__m512i x) {
...
static inline __m512 craftax_wg_uniform_f32x16(uint64_t key, uint64_t base) {
...
static inline void craftax_smoothworld_classify_avx512(
    const CraftaxSmoothGenConfig* config,
    CraftaxThreefryKey tree_uniform_key,
    float* water,
    float* mountain,
...
static inline void craftax_smoothworld_ore_avx512(
    const CraftaxSmoothGenConfig* config,
    int ore_index,
    CraftaxThreefryKey ore_key,
    uint8_t map[CRAFTAX_WG_MAP_SIZE][CRAFTAX_WG_MAP_SIZE]
...
static inline void craftax_smoothworld_lava_avx512(
    const CraftaxSmoothGenConfig* config,
    const float* mountain,
    const float* tree_noise,
    uint8_t map[CRAFTAX_WG_MAP_SIZE][CRAFTAX_WG_MAP_SIZE],
...
static inline void craftax_generate_floor_from_key(
    CraftaxThreefryKey key,
    int level,
...
static inline void craftax_generate_world_from_key_lazy(
    CraftaxThreefryKey rng,
    CraftaxWorldState* out,
    bool lazy
) {
...
static inline void craftax_encode_compact_map_base_observation(
    const CraftaxWorldState* state,
    uint8_t* obs
) {
    const int channels = CRAFTAX_WG_PACKED_CHANNELS_PER_CELL;
...
static inline void craftax_encode_mobs3_compact(
    const CraftaxWorldState* state,
    const CraftaxWGMobs3* mobs,
    int mob_class_index,
    uint8_t* obs
...
static inline void craftax_encode_mobs2_compact(
    const CraftaxWorldState* state,
    const CraftaxWGMobs2* mobs,
    int mob_class_index,
    uint8_t* obs
...
static inline void craftax_encode_compact_observation(
    const CraftaxWorldState* state,
    uint8_t* obs
) {
...
// ============================================================
// ===== standalone harness (not from PufferLib) =====
// ============================================================
// Standalone CLI harness: bench (multithreaded random-action throughput) and
// hash (single-thread deterministic FNV-1a trajectory hash, the
// determinism/regression anchor). This fragment is compiled both against the
// amalgamated sources and against the original PufferLib headers to verify
// the amalgamation is faithful.

#include <omp.h>

static inline uint32_t cf_xorshift32(uint32_t* s) {
    uint32_t x = *s;
    x ^= x << 13;
    x ^= x >> 17;
    x ^= x << 5;
...
// Mirrors binding.c my_vec_init: shared state arena, env->rng = index,
// env->seed = seed_offset + index, reset pool / world pool disabled (exact
// per-seed determinism).
static void cf_vec_init(CfVec* v, int num_envs, uint64_t seed, int parallel) {
    v->num_envs = num_envs;
    v->envs = (Craftax*)calloc((size_t)num_envs, sizeof(Craftax));
    v->observations =
        (CraftaxObs*)calloc((size_t)num_envs * CRAFTAX_OBS_SIZE, sizeof(CraftaxObs));
    v->actions = (float*)calloc((size_t)num_envs, sizeof(float));
    v->rewards = (float*)calloc((size_t)num_envs, sizeof(float));
    v->terminals = (float*)calloc((size_t)num_envs, sizeof(float));
    v->action_rng = (uint32_t*)calloc((size_t)num_envs, sizeof(uint32_t));
    v->arena = (CraftaxArena*)calloc(1, sizeof(CraftaxArena));
    v->arena->states = (CraftaxState*)calloc((size_t)num_envs, sizeof(CraftaxState));
    if (v->envs == NULL || v->observations == NULL || v->arena == NULL
        || v->arena->states == NULL) {
        fprintf(stderr, "allocation failed for %d envs\n", num_envs);
        exit(1);
    }
    v->arena->num_envs = num_envs;
    v->arena->packet_size = CRAFTAX_ARENA_PACKET_SIZE;
    v->arena->num_packets =
        (num_envs + CRAFTAX_ARENA_PACKET_SIZE - 1) / CRAFTAX_ARENA_PACKET_SIZE;

    craftax_wg_init_cell_templates();  // idempotent global, init before ||-ism

    #pragma omp parallel for schedule(dynamic, 4) if (parallel)
    for (int i = 0; i < num_envs; i++) {
        Craftax* env = &v->envs[i];
        env->num_agents = 1;
        env->rng = (unsigned int)i;
        env->seed = seed + (uint64_t)i;
        env->arena = v->arena;
        env->state = &v->arena->states[i];
        env->packet_id = i / v->arena->packet_size;
        env->lane_id = i % v->arena->packet_size;
        env->owns_state_storage = false;
        env->observations = &v->observations[(size_t)i * CRAFTAX_OBS_SIZE];
        env->actions = &v->actions[i];
        env->rewards = &v->rewards[i];
        env->terminals = &v->terminals[i];
        c_init(env);
        craftax_encode_native_observation(env->state, env->observations);
        // Deterministic per-env action stream, independent of seed layout
        // used by the env itself.
        v->action_rng[i] = (uint32_t)(seed ^ ((uint64_t)i * 2654435761ULL)) | 1u;
    }
}

static void cf_vec_free(CfVec* v) {
    free(v->arena->states);
    free(v->arena);
    free(v->envs);
    free(v->observations);
...
// ------------------------------------------------------------
// hash mode: single-thread deterministic rollout, FNV-1a over
// (observations, rewards, dones) each step.
// ------------------------------------------------------------
static int cf_run_hash(int num_envs, int num_steps, uint64_t seed) {
    CfVec v;
    cf_vec_init(&v, num_envs, seed, /*parallel=*/0);

    uint64_t h = 0xcbf29ce484222325ULL;
    // Fold in the post-reset observations so worldgen itself is anchored.
    h = cf_fnv1a(h, v.observations,
    ...
    for (int step = 0; step < num_steps; step++) {
        for (int i = 0; i < num_envs; i++) {
            v.actions[i] =
                (float)(cf_xorshift32(&v.action_rng[i]) % CRAFTAX_NUM_ACTIONS);
            c_step(&v.envs[i]);
        }
        h = cf_fnv1a(h, v.observations,
                     (size_t)num_envs * CRAFTAX_OBS_SIZE * sizeof(CraftaxObs));
        h = cf_fnv1a(h, v.rewards, (size_t)num_envs * sizeof(float));
        h = cf_fnv1a(h, v.terminals, (size_t)num_envs * sizeof(float));
        for (int i = 0; i < num_envs; i++) total_reward += (double)v.rewards[i];
    ...
    printf("completed_episode_reward=%.3f total_reward=%.3f "
           "total_achievements=%.0f\n", ep_ret, total_reward, ach);
...
// ------------------------------------------------------------
// bench mode: OpenMP over envs, deterministic per-env random actions,
// auto-reset on done (inside c_step), reports steps/sec.
// ------------------------------------------------------------
static int cf_run_bench(int num_envs, int iters, int threads, uint64_t seed) {
    ...
    CfVec v;
    cf_vec_init(&v, num_envs, seed, /*parallel=*/1);
    ...
    printf("envs=%d iters=%d threads=%d\n", num_envs, iters, used_threads);
    printf("init %.3fs  bench %.3fs  SPS=%12.0f  (%.2f us/step/env)\n",
           t_init, dt, sps, dt / (double)iters / (double)num_envs * 1e6);

    // Cheap liveness stats so a silent logic break is visible in bench too.
    double episodes = 0.0, ep_ret = 0.0;
    for (int i = 0; i < num_envs; i++) {
        episodes += (double)v.envs[i].log.n;
        ep_ret += (double)v.envs[i].log.score;
    }
    printf("episodes_completed=%.0f completed_episode_reward=%.3f\n",
           episodes, ep_ret);
...
static void cf_usage(const char* prog) {
    fprintf(stderr,
            "usage: %s bench [--envs N] [--iters M] [--threads T] [--seed S]\n"
            "       %s hash  [--envs N] [--steps M] [--seed S]\n",
            prog, prog);
}

int main(int argc, char** argv) {
    // Part of the env API; the harness manages storage/reset itself but keeps
    // these referenced so the full lifecycle surface stays compiled.
    (void)c_reset;
    (void)c_close;
    if (argc < 2) {
        cf_usage(argv[0]);
    ...
    int envs = !strcmp(mode, "hash") ? 64 : 8192;
    int iters = 1000;
    int steps = 2000;
    int threads = 0;
    uint64_t seed = 42;
    ...
    if (envs <= 0 || iters <= 0 || steps <= 0) {
        cf_usage(argv[0]);
        return 1;
    }

    if (!strcmp(mode, "bench")) return cf_run_bench(envs, iters, threads, seed);
    if (!strcmp(mode, "hash")) return cf_run_hash(envs, steps, seed);
    cf_usage(argv[0]);
    return 1;
}
